...
class TestShippingCommandsDeny:
    ...
    def test_shipping_commands_deny(self, run_gemini_hook, cmd):
        result = run_gemini_hook(tool_name="shell", command=cmd)
        assert result.returncode == 0
        data = json.loads(result.stdout)
        assert data["decision"] == "deny"
        assert "Security scan required" in data["reason"]
...
class TestUnknownToolAllows:
    def test_unknown_tool_allows(self, run_gemini_hook):
        result = run_gemini_hook(tool_name="read_file", command="")
        assert result.returncode == 0
        data = json.loads(result.stdout)
        assert data["decision"] == "allow"
