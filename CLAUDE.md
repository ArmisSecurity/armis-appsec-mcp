...
- `single_flight.SingleFlight` → both providers are entered by **several threads at once** (the MCP SDK starts each request as its own task and `server._run_scan` uses `asyncio.to_thread`), so `get_header()` is coalesced: one credential attempt per batch of callers, and its outcome — success *or* failure — is shared with the threads that waited on it. Unsynchronized this is N POSTs to `/auth/token` (measured: 24 concurrent scans → 24 exchanges, 15 of them HTTP 429, 15 files never scanned), N replays of a **rotated** refresh token (reuse detection revokes the token family), or N concurrent device flows (N browser prompts for one scan). Sharing the failure is deliberate — replaying a rate-limited request is what extends the lockout. A caller arriving *after* an attempt finished gets a fresh one, so nothing goes sticky. `resolve` runs with the lock released, so `invalidate()` never blocks on the network.
...
2. Agent calls `git commit`. `hooks/pre_commit_scan.py` fires (PreToolUse on Bash), computes the **current** staged hash, and allows the command only if it equals the stored hash. Stale passes (someone staged more code since scanning) are rejected. Because the hook runs *before* the command, `check_gate` also denies any commit that would record more than the hashed index (`hook_core._index_changing_commit`): an index-mutating git subcommand in the same command line (`git add -A && git commit`), `-a`/`-i`/`-o`/`-p`, pathspecs, or any commit option outside the allowlist. A commit in the same command as a push/PR still gets the hash check.
