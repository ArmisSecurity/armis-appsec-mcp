...
- `auth.init_auth()` → picks the credential path once at startup. **Both** `ARMIS_CLIENT_ID`+`ARMIS_CLIENT_SECRET` set → `auth.JWTAuth` (client-credentials, takes priority, unchanged — backward compatible). Exactly one set → a partial-config error. **Neither** set → `auth.SharedCacheAuth` (shared token cache + Device Auth, PPSC-1038).
...
| `ARMIS_TENANT_ID` | optional | Tenant to authenticate against. Required **only** when the plugin itself starts the browser Device Auth flow (no client creds and an empty `~/.armis/.sessions`). Ignored by the client-credentials path. |
